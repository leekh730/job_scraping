...
db_url = 'mongodb://192.168.219.124:27017/' # ip는 계속 바뀐다. 그러므로 항상 ip주소를 재확인
...
for keyword in keywords:
    search_1 = driver.find_element(By.ID, 'srcKeyword').send_keys(keyword+Keys.ENTER) # 키워드 검색창 선택 후 키워드 입력, 엔터
    for listtitle in range(1,11):
        try:
            ...
            clicktitle = driver.find_element_by_xpath(t_xpath).click() # 클릭으로 오픈
